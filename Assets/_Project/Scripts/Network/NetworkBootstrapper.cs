...
public class NetworkBootstrap : MonoBehaviour
{
    ...
    private void OnClientConnected(ulong clientId)  => Debug.Log($"[Network] 접속: {clientId}</color>");
    private void OnClientDisconnect(ulong clientId) => Debug.Log($"[Network] 해제: {clientId}</color>");
    private void OnServerStarted()                  => Debug.Log("[Network] 서버 시작</color>");
}